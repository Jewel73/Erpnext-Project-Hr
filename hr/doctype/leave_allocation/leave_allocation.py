...
def auto_leave_allocation():
	print("*************************CRON*******************")

	LeaveAllocation = frappe.db.sql("""
		select * from `tabLeave Allocation`
		""")
	print("*********************Leave Allocation*******************")

	updateLeaveAllocation = frappe.db.sql("""
		UPDATE `tabLeave Allocation` SET from_date = MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1), to_date = LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 12-MONTH(CURDATE()) MONTH))
		""")

	print(LeaveAllocation)


	print("*********************Leave Allocation Update*******************")
