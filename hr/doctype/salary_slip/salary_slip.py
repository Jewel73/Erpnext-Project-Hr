...
#--------------------------------------------------------------------------------------------------
		x = payment_days_query(self.emp_id, self.start_date, self.end_date,self.leave_without_pay) 
		payment_days = x
		return payment_days
...
def payment_days_query(emp_id, start_date, end_date, lwp):
	count_att = frappe.db.sql("SELECT attendance_date FROM tabAttendance WHERE uid='"+str(emp_id)+"' AND attendance_date BETWEEN '"+str(start_date)+"' AND '"+str(end_date)+"' AND status = 'Present'")	
	#count_log = frappe.db.sql("SELECT DISTINCT(date) FROM tabLog WHERE uid='"+emp_id+"' AND date BETWEEN '"+str(start_date)+"' AND '"+str(end_date)+"'")
	count_withpay= frappe.db.sql("SELECT total_leave_days FROM `tabLeave Application` WHERE status='Approved' AND leave_type!='Leave Without Pay' AND emp_id='"+emp_id+"'")
	sum = 0.0
	for i,d in enumerate(count_withpay):
		x = list(count_withpay[i])
		sum = sum+x[0]
	print("---------------FROM salary_slip.py------------------------------")
	#return len(count_att)+len(count_log)+lwp+int(sum)
	return len(count_att)+lwp+int(sum)
