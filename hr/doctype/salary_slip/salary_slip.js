...
//-----------------------------
	...
	//	var absent = frm.doc.total_working_days-frm.doc.payment_days-frm.doc.leave_without_pay;   
       //     	frm.set_value('absent',absent);
		//alert(frm.doc.emp_id);
//		alert(frm.doc.payment_days);
		var emp_id=frm.doc.emp_id;
		var start_date = frm.doc.start_date;
		var end_date=frm.doc.end_date;
		var first= new Date(start_date);
		var last=new Date(end_date);
		var formate_first= first.getFullYear()+"-"+(first.getMonth()+1)+"-"+first.getDate();
		var formate_last= last.getFullYear()+"-"+(last.getMonth()+1)+"-"+last.getDate();
		if(start_date!=undefined && end_date!=undefined){
//			get_present_day(frm,emp_id,formate_first,formate_last);
		}
		if (frm.doc.employee!=undefined){	
			loadSalary(frm,frm.doc.employee);
		}
...
function loadSalary(frm,emp){
var branchName=[];
frappe.call({
method: "erpnext.hr.doctype.salary_slip.mysalary.get_user_salary",
args:{"emp":emp},
callback: function(r, rt) {
var sal=r.message;
var salr = parseInt(sal[0]);
console.log("----------------------"+salr+1);
frm.set_value("salary",salr);
...
//---------------------------
function get_present_day(frm,emp_id, start_date, end_date){
var branchName=[];
frappe.call({
method: "erpnext.hr.doctype.salary_slip.mysalary.payment_days_query",
callback: function(r, rt) {
var present_day=r.message;
