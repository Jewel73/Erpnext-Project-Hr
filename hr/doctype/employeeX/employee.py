...
#	    conn = zk.connect()
#	    print(type(int(uid)))
#	    conn.set_user(uid=int(uid), name=name, privilege=const.USER_DEFAULT, password=password, group_id='',user_id=uid, card=int(card))
#	    print("-------------------USer Added--------------------------")
#	    conn.disable_device()
#	    conn.enable_device()
...
def salary_assign(employee):
	...
	assign.update({
		...
		"salary_structure": empl.salary_structure,
		"from_date": empl.effective_date,
...
def auto_leave_allocation_table():
	...
	updateLeaveAllocation = frappe.db.sql("""
		UPDATE `tabLeave Allocation Table` SET from_date = CURDATE(), to_date = DATE_ADD(CURDATE(), INTERVAL 12 MONTH)
		""")
