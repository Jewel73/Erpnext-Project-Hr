...
/*   Author:Shariful Islam
    ...
    var departmentName=[];
    function loadDepartment(frm,d_name,branch_name_value){
        departmentName=[];
        frappe.call({
            method: "erpnext.hr.doctype.employee.bdGeoLocation.get_all_department",
            args: {"bname":branch_name_value},
            callback: function(r, rt) {
                if(r.message) {
                    ...
                    {
                        if (frm.doc.branch_name=="Sales & Marketing") {
                            frm.toggle_display('department', false);
...
function loadTerritory(frm,field_name,d_name){
    ...
    frappe.ui.form.on("Employee", {
        branch_name: function(frm) {
            loadDepartment(frm,"department",frm.doc.branch_name);
        }
    });
...
//          frm.toggle_display('territory_name', true);  
		...
		frm.toggle.display('section', false);
		frm.toggle_display('sub_section', false);  
