...
def auto_leave_allocation_table():
	print("*************************CRON*******************")

	print("*********************Leave Allocation Table*******************")


	updateLeaveAllocation = frappe.db.sql("""
		UPDATE
		`tabLeave Allocation Table`
		SET
		from_date = MAKEDATE(EXTRACT(YEAR FROM CURDATE()), 1),
		to_date = LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 12-MONTH(CURDATE()) MONTH))
		WHERE
		cocola_leave_policy is NOT null
		""")

	print(updateLeaveAllocation)



def auto_delete_extra_leave_allocation_table():
	print("*************************CRON*******************")

	print("*********************Delete Leave Allocation Table*******************")

	deleteExtraLeave = frappe.db.sql("""
		DELETE `tabLeave Allocation Table` , `tabLeave Allocation` 
		FROM `tabLeave Allocation Table` left join `tabLeave Allocation`
		On `tabLeave Allocation Table`.parent = `tabLeave Allocation`.employee 
		where CURDATE() >= `tabLeave Allocation Table`.to_date AND `tabLeave Allocation Table`.cocola_leave_policy is  null
		AND CURDATE() >= `tabLeave Allocation`.to_date
		
		""")

	print(deleteExtraLeave)